...
- [Документация](#документация)
  - [Аргументы/параметры скрипта](#аргументыпараметры-скрипта)
  - [Типы архивов](#типы-архивов)
  - [Возможные структуры архивов с частями внутри](#возможные-структуры-архивов-с-частями-внутри)
    - [Дополнительные нюансы с архивами:](#дополнительные-нюансы-с-архивами)
  - [Умное переименование](#умное-переименование)
    - [Какие есть режимы переименования](#какие-есть-режимы-переименования)
  - [Удаление дубликатов](#удаление-дубликатов)
  - [Выявленные нюансы](#выявленные-нюансы)


## Аргументы/параметры скрипта

1. `targetPath` | строка | **обязательный**
   - путь к файлу-архиву, который необходимо распаковать или к папке с файлами-архивами
2. `archiverPath` | строка | не обязательный
   - путь к исполняемому файлу архиватора, который будет обрабатывать файлы
   - по умолчанию будет использоваться стандартный путь, куда устанавливается 7-Zip
   - смотрите поддерживаемые архиваторы в [README](../README_RU.md) согласно платформе, на которой используете скрипт
3. `outputFolderPath` | строка | не обязательный
   - путь к папке в которую будут помещаться распакованные архивы
   - по умолчанию будут распаковывать в ту же папку, в которой находятся исходные файлы-архивы
4. `smartRenameMode` | число | не обязательный
   - режим переименования папок с распакованными файлами, в основном заменяет точки на пробелы, но не только это
   - по умолчанию число `0` - никакого переименования не происходит
   - подробнее ниже в разделе умного переименования
5. `duplicatesProcessMode` | число | не обязательный
   - режим анализа и поиска дублирующихся файлов при распаковке частей архивов внутри основного архива с релизом
   - по умолчанию число `0` - анализ на наличие дублирующихся файлов не происходит и при распаковке частей архивов файлы с одинаковым именем пропускаются (то есть в 7z используется аргумент `-aos`)
   - подробнее ниже в разделе про дубликаты
6. `overwriteExisting` | переключаемый | не обязательный
   - наличие этого параметра приводит к удалению папки с таким же названием, как у "обработанной" распакованной папки, если такая есть в `outputFolderPath`
   - по умолчанию параметр выключен, то есть если будет папка с таким же именем как у итоговой распакованной папки, то к итоговой распакованной папке будет добавлять числовой индекс в конце
7. `deleteArchiveAfterUnpack` | переключаемый | не обязательный
   - наличие этого параметра приводит к удалению архива после распаковки. Удаление полностью с диска, а не перемещение в Корзину
   - по умолчанию параметр выключен, то есть исходные архивы-релизы останутся там же, где они были изначально

## Типы архивов

Есть специальные правила, которыми, наверное, руководствуются релиз-группы при создании релиза (архивов с релизами). Для разного типа контента эти правила разные и с ними можно ознакомиться на сайте [scenerules](https://scenerules.org/)

Об этом сайте я узнал буквально после публикации данного репозитория. К сожалению у меня нет возможности досконально ознакомиться с правилами оформления/архивации релизов на ресурсе [scenerules](https://scenerules.org/). Да и к тому же варез сцены это приватное место, и к оригинальным варез сценам есть доступ только у небольшого количества людей. Из-за этой приватности хакеры/крякеры/реверс инженеры и другие типы релиз групп не имеют возможности публиковать свои работы на варез сценах и создают свои релизы подражая тому, как это делают релиз группы в варез сценах. Поэтому их релизы/архивы могут отличаться.

Я старался делать скрипты максимально универсальными, чтобы они могли корректно обработать любую структуру архивов, а не только то, что прописано в официальных правилах варез сцен.  По моему опыту вот с чем мы можем столкнуться чаще всего:
...
### Дополнительные нюансы с архивами:
...
Зачастую в архивах частях, которые находятся внутри основного архива с релизом, дублируются файлы `.nfo` и `.diz`. То есть у них у всех одинаковое имя. Обычно это те же самые файлы, что находятся в папке с этими частями архива, но не факт, что так будет всегда и возможно какой-то из этих файлов уникальный и имеет дополнительную важную информацию.

Параметры:
- `0` - удалять дубликаты
  - с таким параметром при распаковке с архиватором 7-Zip используется аргумент `-aos`, что приводит к пропуску распаковки файла из архива (то есть файл не извлекается), если файл с таким именем есть в папке для распаковки
- `1` - сохранять дубликаты
  - с таким параметром при распаковке с архиватором 7-Zip используется аргумент `-aou`, что приводит к переименованию файла, если файл из распаковываемой части присутствует среди распакованных файлов. 7-Zip при это к файлам-дубликатом добавляет числовой индекс к конце имени в формате `<оригинальное имя>_<число от 00 и больше>.<оригинальное расширение>`
  - далее происходит вычисление хэш-сумм всех файлов, начиная от корня папки в которую распаковался основной архив-релиз
  - сначала вычисляются хэш-суммы файлов `.nfo`, `.diz`, `.txt` у которых нет числового индекса в конце имени, т.к. наиболее вероятно что они "более оригинальные". Потом вычисляются хэш суммы остальных файлов и сравниваются с хэш-сумма уже вычисленных файлов и хэш-сумма вычисленного файла совпадает - он удаляется
  - анализируются все файлы, находящиеся в корне распакованной папки-релиза и всех подпапках, поэтому если у вас слабый компьютер (процессор и накопитель) или релиз-файлы много весят - этот параметр ощутимо увеличит время распаковки

Если все же вы не понимаете как это работает - попробуйте вручную пораспаковывать и исследовать (штук 10-15) внутренности разных архивов с варез-сцен с вложенными частями-архивами и посмотреть, что находится внутри них.

Маловероятно, что в релизе могут быть файлы с одинаковым именем, но разным содержанием, но такое не исключено. Поэтому если вы параноите на эту тему - параметр `1` вам поможет.

## Выявленные нюансы

1. Не используйте `rar.exe` и `unrar.exe` для распаковки zip-архивов. Они предназначены для работы только с rar-архивами и zip-архивы будут рассматривать как самораспаковывающиеся SFX-архивы и будут с ними работать неправильно.
...
4. Powershell автоматически распаковывает коллекции, если в них 1 элемент. Поэтому при конкатенации коллекций может возникнуть проблема, т.к. может произойти попытка сложения элемента с массивом. Чтобы предотвратить это - необходимо при конкатенации коллекций оборачивать их в оператор создания массивов `@($list)`